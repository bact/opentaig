...
def search_huggingface(kind: str, term: str, size: int, warnings: list) -> list[dict]:
    ...
    resp = requests.get(
        f"{HF_API}/{endpoint}", params={"search": term, "limit": size}, timeout=30
    )
    if resp.status_code != 200:
        warnings.append(
            f"huggingface {endpoint} search {term!r} failed: HTTP {resp.status_code}"
        )
        return []
    return resp.json()
...
def main() -> None:
    ...
    parser.add_argument(
        "--size", type=int, default=20, help="results to fetch per keyword"
    )
    parser.add_argument(
        "--min-stars",
        type=int,
    ...
    parser.add_argument(
        "--min-readme-chars",
        type=int,
        default=DEFAULT_MIN_README_CHARS,
        help="npm only",
    )
    parser.add_argument(
    ...
    if args.registry == "npm":
        ...
        if not token:
            raise SystemExit(
                "GITHUB_TOKEN is not set -- npm hits are resolved to GitHub repos "
        ...
        if args.registry == "npm":
            ...
            for hit in hits:
                ...
                candidates[full_name] = {
                    ...
                    "description": (
                        item.get("description") or pkg.get("description") or ""
                    ).strip(),
            ...
            for hit in hits:
                ...
                if not hf_id:
                    continue
                kind_path = (
                    "spaces" if args.registry == "huggingface-spaces" else "models"
                )
                ...
                candidates[full_name] = {
                    ...
                    "pushed_at": hit.get("lastModified", "")
        ...
        log_rows.append(
            {
                ...
                if args.registry == "npm"
                else "n/a (registry API, no README check)",
