...
def combos(dim_a: str, dim_b: str) -> list[str]:
    if dim_a not in DIMENSIONS:
        raise SystemExit(
            f"unknown dimension {dim_a!r}; choices: {', '.join(DIMENSIONS)}"
        )
    if dim_b not in DIMENSIONS:
        raise SystemExit(
            f"unknown dimension {dim_b!r}; choices: {', '.join(DIMENSIONS)}"
        )
...
def main() -> None:
    ...
    parser.add_argument(
        "--dims", help="two dimension names, comma-separated, e.g. target,action"
    )
    parser.add_argument(
        "--list-dims",
        action="store_true",
        help="print dimension names and their terms, then exit",
    )
    parser.add_argument(
        "--limit", type=int, default=None, help="cap the number of phrases printed"
    )
    ...
    if len(parts) != 2:
        raise SystemExit(
            "--dims must name exactly two dimensions, e.g. --dims target,action "
    ...
    if not args.show_tried:
        ...
        print(
            f"{len(phrases)} total combos, {new_count} not yet in {args.log_path}",
            file=sys.stderr,
        )
